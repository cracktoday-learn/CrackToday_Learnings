...
import { Trophy, Clock, CheckCircle, AlertCircle, ArrowRight, Users, Target, TrendingUp, Star, Share2, MessageCircle, RotateCcw, BookOpen, Flame, BarChart3, Zap, Download } from "lucide-react";
...
export function TestEvaluation() {
  ...
  const handleDownloadPDF = () => {
    ...
    const percentile = calculatePercentile();
    const testName = test?.name || `Test ${testNumber}`;
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Test Result - ${testName}</title>
        <style>
          body { font-family: Arial, sans-serif; padding: 40px; color: #333; }
          .header { text-align: center; margin-bottom: 30px; }
          .header h1 { color: #4f46e5; margin: 0; }
          .header p { color: #666; margin: 5px 0; }
          .score-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 15px; text-align: center; margin-bottom: 30px; }
          .rank { font-size: 48px; font-weight: bold; margin: 10px 0; }
          .stats { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 30px; }
          .stat-box { background: #f8fafc; padding: 20px; border-radius: 10px; text-align: center; }
      ...
      <body>
        ...
        <div class="score-card">
          <p style="font-size: 18px; margin: 0;">Your Rank</p>
          <div class="rank">#${userRank}</div>
          <p style="font-size: 16px; margin: 10px 0;">out of ${allAttempts.length} participants</p>
          <p style="font-size: 20px; margin-top: 20px;">${percentage}% Score • ${percentile}th Percentile</p>
        </div>
        
        <div class="stats">
          <div class="stat-box">
            <div class="stat-label">Correct</div>
            <div class="stat-value" style="color: #10b981;">${calculatedMetrics.correct}</div>
          </div>
          <div class="stat-box">
            <div class="stat-label">Wrong</div>
            <div class="stat-value" style="color: #ef4444;">${calculatedMetrics.wrong}</div>
          </div>
          <div class="stat-box">
            <div class="stat-label">Skipped</div>
            <div class="stat-value">${calculatedMetrics.skipped}</div>
          </div>
          <div class="stat-box">
            <div class="stat-label">Time Taken</div>
            <div class="stat-value">${userAttempt ? formatTime(userAttempt.time_taken) : "0m 0s"}</div>
          </div>
        </div>
        
        <div class="footer">
          <p>Generated by CrackToday - Your UPSC Preparation Partner</p>
          <p>https://cracktoday.com</p>
        </div>
        ...
        >
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl font-semibold transition-colors"
            >
